...
#単一検索
      ...
      @products = Kaminari.paginate_array(Product.includes(:scenes).where('scene = ?', params[:scene]).references(:scene).order("products.created_at DESC")).page(params[:page]).per(@per)
      @current_page = params[:page].to_i

    elsif chara.present? #キャラクター別
      @products = Kaminari.paginate_array(Product.includes(:charas).where('chara = ?', params[:chara]).references(:chara).order("products.created_at DESC")).page(params[:page]).per(@per)
      @current_page = params[:page].to_i
      ...
      @products = Kaminari.paginate_array(Product.includes(:balloon_types).where('balloon_type = ?', params[:balloon_type]).references(:balloon_type).order("products.created_at DESC")).page(params[:page]).per(@per)
      @current_page = params[:page].to_i
#条件なし
    else
...
#終了
  ...
  def ranking
    @per = 5
    @products = Product.page(params[:page]).per(@per).order("number DESC")
    @current_page = params[:page].to_i
  end
#http://localhost:3000/api/budget.json?page=1&min=100&max=100000 みたいな
  def budget
    min = params[:min]
    max = params[:max]
    @products = Product.where('price >= ?',params[:min]).where('price <= ?',params[:max]).page(params[:page]).per(@per).order("created_at DESC")
    @current_page = params[:page].to_i
  end
